...
import { useAuthStore } from '@/store/authStore'
import { updateUserDetails, requestEmailUpdate, verifyEmailUpdate } from '@/services/authService'
import { ArrowLeft, User, Mail, Phone, AtSign, Calendar, Venus, Check, Loader2 } from 'lucide-react'
    ...
    return (
        <div className="flex items-center gap-4 px-4 py-4">
            ...
            <div className="min-w-0">
                ...
                <p className="text-sm font-medium text-[#1E2A3A] truncate">{value || '—'}</p>
            </div>
        </div>
    )
}
...
export default function PersonalInfoPage() {
    ...
    const handleRequestEmailOtp = async () => {
        ...
        if (email === user?.email) return toast.error('New email is same as current email')
        ...
        <div className="min-h-screen bg-white">
            ...
            <div className="max-w-md mx-auto px-4 pb-12">
                ...
                <div className="flex flex-col items-center text-center gap-2 py-6">
                    ...
                    <div>
                        <h2 className="text-lg font-bold text-[#1E2A3A]">{user?.full_name}</h2>
                        {user?.username && (
                ...
                <div className="flex items-center justify-between mb-2">
                    ...
                    {editMode !== 'details' ? (
                        <button onClick={openEdit} className="text-xs font-semibold text-[#F97316]">EDIT</button>
                    ) : (
                ...
                <div className="bg-gray-100 rounded-2xl overflow-hidden mb-4">
                    {editMode === 'details' ? (
                        <div className="p-4 space-y-3">
                            ...
                            <div>
                                <label className="text-xs text-gray-400 tracking-wide">FULL NAME</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="text-xs text-gray-400 tracking-wide">PHONE NUMBER</label>
                                <input
                                    type="tel"
                            ...
                            <div>
                                <label className="text-xs text-gray-400 tracking-wide">GENDER</label>
                                <select
                                    value={form.gender}
                            ...
                            <div>
                                <label className="text-xs text-gray-400 tracking-wide">DATE OF BIRTH</label>
                                <input
                                    type="date"
                                    value={form.DOB}
                        ...
                        <>
                            <InfoRow icon={<User size={16} className="text-orange-500" />} label="FULL NAME" value={user?.full_name} />
                            <div className="border-t border-gray-200" />
                            <InfoRow icon={<AtSign size={16} className="text-purple-400" />} label="USERNAME" value={user?.username ? `${user.username}` : undefined} />
                            <div className="border-t border-gray-200" />
                            <InfoRow icon={<Phone size={16} className="text-blue-400" />} label="PHONE NUMBER" value={user?.phone_number} />
                            <div className="border-t border-gray-200" />
                            <InfoRow icon={<Venus size={16} className="text-pink-400" />} label="GENDER" value={user?.gender ? user.gender.charAt(0).toUpperCase() + user.gender.slice(1) : undefined} />
                            <div className="border-t border-gray-200" />
                            <InfoRow icon={<Calendar size={16} className="text-green-500" />} label="DATE OF BIRTH" value={formatDOB(user?.DOB || '')} />
                        </>
                    )}
                </div>
                ...
                <div className="flex items-center justify-between mb-2">
                    ...
                    {editMode !== 'email' && editMode !== 'otp' ? (
                        <button onClick={() => { setEditMode('email'); setNewEmail('') }}
                            className="text-xs font-semibold text-[#F97316]">CHANGE</button>
                    ) : (
                        <button
                            onClick={() => { setEditMode(null); setNewEmail(''); setOtp('') }}
                            disabled={saving}
                            className="text-xs text-gray-400 disabled:opacity-40"
                        >
                            Cancel
                ...
                <div className="bg-gray-100 rounded-2xl overflow-hidden">
                    <InfoRow icon={<Mail size={16} className="text-blue-500" />} label="EMAIL" value={user?.email} />

                    {editMode === 'email' && (
                        <div className="px-4 pb-4 space-y-3">
                            <div className="border-t border-gray-200 mb-3" />
                            <div>
                                <label className="text-xs text-gray-400 tracking-wide">NEW EMAIL</label>
                                <input
                                    type="email"
                                    value={newEmail}
                    ...
                    {editMode === 'otp' && (
                        <div className="px-4 pb-4 space-y-3">
                            ...
                            <p className="text-xs text-gray-500">
                                OTP sent to <span className="font-semibold text-[#1E2A3A]">{pendingEmail}</span>
                            </p>
                            <div>
                                <label className="text-xs text-gray-400 tracking-wide">ENTER OTP</label>
                                <input
                                    type="text"
                                    inputMode="numeric"
                                    maxLength={6}
